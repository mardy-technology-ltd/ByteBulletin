...
export function TrendingWidget({ initialArticles }: TrendingWidgetProps) {
  const [trendingArticles, setTrendingArticles] = useState<any[]>(initialArticles || []);
  ...
  if (!isLoading && (!trendingArticles || trendingArticles.length === 0)) return null;
